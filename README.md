...
## DOC
- [CURDConfig类](doc/CURDConfig.md)
- [CURDSite类](doc/CURDSite.md)
- [ShowView类](doc/ShowView.md)

## 配置与使用
#### 1. 将组件的curd应用放到项目目录下
#### 2. settings.py中注册curd应用
...
#### 3. 在目标应用（比如blog应用）下创建`curd.py`
- 这个文件用于配置后台管理页面的功能，你也可以将每一个模型的配置放在单个文件下

#### 4. 在配置文件中你要干下面几件事儿
1. 从curd组件中导入`sites`模块
2. 从你的应用模型文件中导入`模型类`
	- 你还可以定义`ModelForm`类，不过建议你单独将这些类放在其他文件中
3. 如果你要配置组合搜索功能，你需要从`curd.service.views`中导入`SearchOption`对象。
4. 使用`sites.site.register()`方法注册模型类，并为模型类绑定配置类
